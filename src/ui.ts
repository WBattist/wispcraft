...
<div class="settings-ui hidden" id="settings_ui">
	...
	wispInput.onchange = async () => {
		if (saveTi != -1) clearTimeout(saveTi);
		try {
			...
			saveStatus.textContent = `Wisp server set successfully!`;
		} catch (e) {
            // FIXED: Use textContent to prevent XSS
			saveStatus.textContent = `An error occured: ${String(e)}`;
		}
		saveTi = setTimeout(() => {
	...
	// REFACTOR: This logic is duplicated from index.ts. It should be extracted
	...
	accountSelect.onchange = async () => {
		...
		const accounts = JSON.parse(localStorage["wispcraft_accounts"] || "[]") as TokenStore[];
		const account = accounts.find((acc) => acc.username === accountSelect.value);
		if (account) {
			try {
				try {
				...
				console.error("Failed to authenticate account, removing:", e);
				removeAcc();
			}
		}
	...
	const removeAcc = () => {
		...
		const accounts = JSON.parse(localStorage.getItem("wispcraft_accounts") || "[]");
		const accountIndex = accounts.findIndex((acc: TokenStore) => acc.username === accountSelect.value);
		if (accountIndex > -1) {
			accounts.splice(accountIndex, 1);
			localStorage.setItem("wispcraft_accounts", JSON.stringify(accounts));
	...
	addButton.onclick = async () => {
		try {
            ...
            // This is mostly safe as qrSvg is generated locally, but care is still needed.
            ...
            const accounts = JSON.parse(localStorage.getItem("wispcraft_accounts") || "[]");
			const newAccEntry: TokenStore = {
				username: authstore.user.name,
				token: authstore.yggToken,
				ms: authstore.msToken,
			};
			const existingAccountIndex = accounts.findIndex((acc: TokenStore) => acc.username === authstore.user?.name);
			if (existingAccountIndex > -1) {
				accounts[existingAccountIndex] = newAccEntry;
			} else {
			...
			// FIXED: Use textContent to prevent XSS vulnerability.
            const errorElement = document.createElement('code');
            errorElement.textContent = String(e);
            accountStatus.textContent = 'An error occured: ';
            accountStatus.appendChild(errorElement);
			addButton.disabled = false;
		}
	};
